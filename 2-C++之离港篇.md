...
> Q: 能不能只有别名？  
> A: 只有别名，别名就变成了真实姓名.只有别名也是无法进行命名的。
...
`a` 是 `x` 的别名。`b` 是 `y` 的别名。 里面操作的就是实际的参数了。
...
看起来传入的是实参 `x`, `y` 实际上 `a` 是 `x` 的引用，`b` 是 `y` 的引用。
...
需求：定义一个引用y，y是x的引用，然后打印x和y的值。将y的值更改之后再次打印，x和y的值。
...
`x` 变成了一个常量，无法进行更改。再赋值其他数字，编译时就会报错。

### const与指针类型
	
- `const int *p = NULL` 完全等价于 `int const *p = NULL`，但是 `int *const p=NULL` 与前两种有区别。[^1]

- 除了常规上面加一处，也可以在前面后面都加：`const int * const p = NULL;` 完全等价于  `int const * const p = NULL;`
...
通过 `#define` 和 `const` 修饰的都可以达成设置常量目的：

- `const` 的优点是，常量有类型，在编译的时候要检查语法错误。
- 而 `#define` 定义的没有数据类型，是宏定义在编译时不再检查语法错误
- 推荐用 `const` 来定义常量
...
// D.
const int a = 3; int const &b = a;
```

>B const 修饰`*p` C const 修饰 `p` D const修饰a的别名b 
指针指向const修饰的变量时,应该是`const int const *p = &a;`
...
>实际的编译之后，名称+参数形成新的函数。来区分两个所谓的同名函数 (即Name mangling)。[^2]
...
subgraph Inline_Function["inline 函数调用"]
A1[调用 inline 函数] --> B1[编译器在调用处展开函数代码]
B1 --> C1[直接执行展开后的代码]
C1 --> D1[继续后续程序]
end

subgraph NonInline_Function["普通函数调用"]
A2[调用普通函数] --> B2[保存当前上下文]
B2 --> C2[跳转到函数地址]
C2 --> D2[执行函数体]
D2 --> E2[返回调用点]
E2 --> F2[恢复上下文并继续执行]
...
而内联函数会在编译时将函数体代码和实参代替函数调用语句，省掉了2和4步骤，会节省时间，尤其是循环调用。
...
3. 内联函数：`inline` 效率高但有条件（1.逻辑简单，2.不能是递归）
...
5-4-StrcpyMemoryMange/main.cpp [^4]
```cpp
#include <string.h>
#include <iostream>
using namespace std;
...
[^1]: `NULL` 是为了兼容 C语言代码而存在的，因为 C++ 引入了重载，为了避免指针类型和数值类型混淆，建议使用 `nullptr` 代表空指针而不是 `NULL`.  
[^2]: 每一种编译器的函数重命名(Name mangling)规则不一样，所以在项目中通常不能混用编译器，文中的例子也只是便于理解.  
[^3]: 因为 `inline` 相当于把其他函数复制粘贴到调用处了，一定程度上也会导致编译好的代码体积膨胀.  
[^4]: `strcpy_s` 是 Windows 下特有的安全版本的函数，在非 Windows 下（或者使用 DevC++ 时）可能会编译失败，请将其换成：`strcpy(str, "Hello C++")`