...
1. 打开 [Visual Studio 官网](https://visualstudio.microsoft.com/)，下载 Visual Studio。（注意要下载 Visual Studio 而不是 Visual Studio Code）

2. 安装的过程中选择 Visual C++ 工作负载并安装。

新建项目：

1. 点击 `新建项目`，找到 Visual C++ 下的控制台应用程序，点击下一步

2. 设定一个位置存储项目的文件，填写项目名称，完成创建

> 或者也可以创建空项目，理解程序从无到有。  
>
> 1. 点击 `新建项目`，找到 Visual C++ 下的空项目，点击下一步
> 
> 2. 设定一个位置存储项目的文件，填写项目名称，完成创建

新建文件：

1. 在右侧 `解决方案管理器` 窗口上右键 `源文件` 选项

2. 找到新建文件（而不是添加现有文件）

3. 选择 C++ 源文件，输入文件名称 `xxx.cpp`（如 `demo.cpp`），创建。
...
1. 在 App Store 中搜索 `Xcode`，获取并安装。  

    ![](assets/1/4.jpg)

    安装完毕后打开 Xcode，会提示安装工具链，仅勾选电脑的工具链即可。

2. 打开 [JetBrains CLion](https://www.jetbrains.com/zh-cn/clion/) 官网，并下载安装。

    ![下载 CLion](assets/1/8.jpg)

3. 为 CLion 安装中文语言包

创建项目步骤：

1. 打开 CLion，点击 `New Project`（`新建项目`），选择 `C++ 控制台项目`

2. 设定标准为 `C++11`，填写项目名称，新建项目
...
- 正常的 `system("pause")` 包含于 `stdlib.h`[^1]
...
[^1]: 为了区分 C 和 C++ 两种语言，其实 C++ 中更推荐使用 `cstdlib` 而不是 `stdlib.h`，将上述代码的 `#include <stdlib.h>` 换成 `#include <cstdlib>` 也是能用的。